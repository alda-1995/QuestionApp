<form method="POST" action="{{ route('login') }}" novalidate>
    @csrf
    <x-controls.input label="Correo electronico" type="email" name="email" id="email"
        value="{{ old('email') }}" required autofocus placeholder="correo@ejemplo.com" />
    <x-controls.input type="password" id="password" name="password" placeholder="Escribe tu contraseña" label="Contraseña" value="{{ old('password') }}" required
        autofocus />
    ...
    <div class="mt-4">
        <x-buttons.btn-main type="submit" class="w-full" label="INGRESAR" />
    </div>
</form>